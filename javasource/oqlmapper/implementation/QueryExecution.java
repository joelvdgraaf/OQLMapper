...
public class QueryExecution
{
	...
	public QueryExecution(String templateName, String OQLQuery, IMendixObject parentObj) throws CoreException
	{
		...
		{
			throw new CoreException("The requested template '" + templateName + "' wasn't foud in memory so the query can't be executed. Please check if the templates are loaded in memory and check your log.");
		}
		...
		if(parentObj != null)
		{
			...
			{
				throw new CoreException("The given parent Object '" + parentObj.getType() + "' isn't equal or a specialization of the expected type: " + this.template.getParentObject());
			}
		} else
		{
			throw new CoreException("The requested template '" + templateName + "' could not be executed because the parent object wasn't given.");
		}
	}
	...
	{		
		...
		while(resultIterator.hasNext())
		{
			...
			// Create object and set the association to the parent object as reference
			...
			for(Mapping map : this.template.getMappingList())
			{
				...
				if(value != null)
				{
					log.trace(rowCounter + " Parsing retrieved value for mapping: " + map.getAliasName() + " with parse type: " + map.getParseType());
	 ...
	 * @param context The context to execute the query
